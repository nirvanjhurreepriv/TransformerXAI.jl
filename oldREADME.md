...
```
3. Add TransformerXAI.jl
```julia
add https://github.com/nirvanjhurreepriv/TransformerXAI.jl
```
4. Leave package mode and load the package 
```julia
...
```
4. Install all recorded dependencies and precompile
```julia
instantiate
precompile
```
...
Main function:
```julia
attention_history, tokens = extract_att_weights(bot::ChatBot; input_prompt::String="Once upon a time", desired_layer::Int=1, desired_head::Int=1)
```
Example in Julia shell: 
```julia
using TransformerXAI
...
bot = load_llama_model(model_path, tokenizer_path)
layer = 1
attention_history, token_strings = extract_att_weights(bot, input_prompt = "In a small village, there was a", desired_layer = layer)
```
...
Main function:
```julia
generate_attention_heatmap_matrix(attention, tokens)
```
Example in Julia shell: 
```julia
using TransformerXAI
...
```
The function plots the attention heatmap matrix.
